...
const GROUPS: { title: string; items: NavItem[] }[] = [
  ...
  },
  {
    title: "Pessoas",
    items: [
      { href: "/painel/conferencia", label: "Conferência", icon: Check, badge: true },
      { href: "/painel/participantes", label: "Participantes", icon: Users },
      { href: "/painel/inscricoes", label: "Inscrições", icon: List },
      { href: "/painel/filiacoes", label: "Fichas de filiação", icon: ClipboardNote },
...
function NavLinks({ role, queue, onNavigate }: { role: StaffRole; queue: number; onNavigate?: () => void }) {
            ...
            {items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.badge && queue > 0 ? (
                    <span className="pixel rounded-[3px] bg-warning px-1.5 py-1 text-[0.5rem] text-warning-foreground tabular">{queue}</span>
                  ) : null}
                </Link>
              );
            })}
...
export function PanelSidebar({ role, queue, eventName }: { role: StaffRole; queue: number; eventName: string }) {
  return (
    <aside className="no-print sticky top-0 hidden h-dvh w-64 shrink-0 flex-col gap-6 border-r border-line bg-ink-2/95 px-3 py-5 lg:flex">
      ...
      <div className="no-scrollbar -mx-1 min-h-0 flex-1 overflow-y-auto px-1">
        <NavLinks role={role} queue={queue} />
      </div>
...
export function PanelMobileNav({ role, queue, eventName }: { role: StaffRole; queue: number; eventName: string }) {
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <Button variant="ghost" size="icon-lg" className="relative lg:hidden" aria-label="Abrir menu">
          <Menu />
          {queue > 0 ? <span className="absolute top-2 right-2 size-2 animate-blink rounded-[2px] bg-warning" /> : null}
        </Button>
      </SheetTrigger>
      <SheetContent side="left" className="w-72 gap-6 overflow-y-auto border-line bg-ink-2 p-4">
        ...
        <NavLinks role={role} queue={queue} onNavigate={() => setOpen(false)} />
      </SheetContent>
    </Sheet>
  );
}
