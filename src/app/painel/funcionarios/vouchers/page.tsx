...
export const metadata: Metadata = { title: "Vouchers dos funcionários", referrer: "no-referrer" };

/** Vouchers de todos os funcionários (e dos convidados deles, logo ao lado), dois por linha, para imprimir de uma vez. */
export default async function EmployeeVouchersPage({ searchParams }: PageProps<"/painel/funcionarios/vouchers">) {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <p className="no-print mb-5 flex items-start gap-2 rounded-xl border border-line bg-surface-2 p-3 text-sm text-fg-muted">
        ...
        Dois vouchers por linha, em folha A4: o de cada funcionário e, logo depois, o do convidado dele. Cada um tem o próprio QR
        Code.
      </p>
      {cards.length && voucherEvent ? (
        ...
        <p className="rounded-xl border border-line bg-surface p-6 text-center text-fg-muted">Nenhum funcionário na lista.</p>
      )}
    </div>
  );
}
